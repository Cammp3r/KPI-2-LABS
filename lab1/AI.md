...
Промт - "норм сделал? (https://lh3.googleusercontent.com/gg/ACRwjauRc4X3Arh-qJsAbwWab9QlkEWH7AEpWLAfOKgVK7tT1RleINR8tlCSCAXppIPFjfIIU2OrImeDbhwmBqKY5LYojXJmaFxkDuaJgvkDt-11r2PE9HBkqsTh3xx_chVbSwgOqU7UoPrhbCQBKLrjvpBdw3M1RWnFbU65czxBwaBUWtd0tbiZ8jr37zOa49ojwo6gguoCcXsI551knrcn6VgswNzGe3bTCyZiuA1Sviask9fsa50Qvi1pWAF3vEd1HMyOt15A7Z9vbDQgjf_zg3Ouv4UcuWKfRtw)"

Відподвідь - "У завданні вказано чіткий формат здачі: **«Здача — 2 входи»**.

Здавати роботу потрібно двома обов'язковими частинами:

---

### 1. Посилання на Pull Request (GitHub PR)

Ти створюєш репозиторій на GitHub, робиш окрему гілку (наприклад, `feature/er-model`) і відкриваєш **Pull Request** у `main`. У PR мають бути наступні файли:

* **`README.md`** — опис домену (LeetCode, бізнес-процеси, межі системи).
* **`spec.md`** — детальний текстовий опис усіх сутностей, атрибутів (з типами), зв'язків та критеріїв нормалізації (3NF). Якщо для генерації використовувався ШІ, сюди або в окремий розділ додаються використані промпти.
* **Файл діаграми (код + рендер):**
* Найпростіший варіант — помістити код діаграми прямо в `README.md` або окремий `diagram.md` у блоці коду:
```markdown
```mermaid
erDiagram
    %% твій код діаграми тут
```

```


* GitHub автоматично перетворить цей код на графічну схему прямо в браузері (це закриває вимогу *«текст + рендер»*).


* **Історія комітів:**
* У вимогах є пункт: *«топ-3 виправлення через зміну spec/критеріїв (коміти), а не ручний патч»*.
* Тобто в PR має бути видно історію, як ти покращував схему (наприклад, коміт: `fix(spec): remove redundant FKs in EditorialPost to satisfy 3NF`, потім `fix(diagram): update EditorialPost relation to Submission`). Не роби один єдиний коміт із назвою `fix`.



---

### 2. Заповнений `DEFENSE`

Викладач пропонує два варіанти на вибір:

* **Варіант А:** Заповнити прикріплений у завданні шаблон **Google Doc** (індивідуальна копія, створена для тебе в Google Classroom).
* **Варіант Б:** Створити файл **`DEFENSE.md`** прямо в корені свого репозиторію на GitHub.

**Що обов'язково має бути всередині DEFENSE:**

1. **Чому цей домен:** коротке обґрунтування вибору предметної області алгоритмічної платформи.
2. **Чому така нормалізація:** пояснення рішень щодо структури таблиць (наприклад, чому `Tag` винесений окремо через $M:N$, чому `EditorialPost` посилається на `Submission`, а не дублює мову та задачу, як досягнуто 3NF).
3. **Розділ «Перевірка» (узгодженість spec $\leftrightarrow$ модель):** порівняння або чеклист, який підтверджує, що кожне поле, його назва, тип і кардинальність зв'язку в діаграмі на 100% збігаються з текстом у `spec.md`.

---

### Фінальний крок

В інтерфейс Google Classroom (де прикріплено завдання) ти вставляєш **посилання на створений GitHub PR** та прикріплюєш/заповнюєш **DEFENSE**."